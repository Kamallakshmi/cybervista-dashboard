...
export const NavBody = ({ children, className, visible }: VisibilityProps) => (
  <motion.div
    animate={{
...
export const MobileNav = ({
  ...
  <motion.div
    ...
    transition={{ type: "spring", stiffness: 200, damping: 50 }}
    className={cn(
      "lg:hidden mx-auto w-full max-w-[calc(100vw-2rem)] flex flex-col items-center py-2",
      visible && "bg-white/80 dark:bg-neutral-950/80",
      className
    )}
  >
...
export const MobileNavHeader = ({
  ...
  <div className={cn("flex w-full justify-between", className)}>{children}</div>
);
  ...
  <AnimatePresence>
    {isOpen && (
      <motion.div
        ...
        exit={{ opacity: 0 }}
        className={cn(
          "absolute inset-x-0 top-16 z-50 flex flex-col bg-white dark:bg-neutral-950 px-4 py-8 shadow-xl rounded-lg",
          className
        )}
      >
        {children}
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md text-sm font-bold inline-block text-center transition hover:-translate-y-0.5";
  const variantStyles = {
